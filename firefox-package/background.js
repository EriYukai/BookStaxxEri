...
function logDebug(message, data) {
    ...
    const browserInfo = isFirefox() ? 'Firefox' : 'Chrome';
    const logMessage = `[BookStaxx-${browserInfo} ${timestamp}] ${message}`;
    ...
    // Firefox 전용 추가 디버깅
    if (FIREFOX_DEBUG && isFirefox()) {
        console.trace('Firefox debug trace:', message);
    }
}
...
if (typeof chrome === 'undefined' && typeof browser !== 'undefined') {
  // Firefox environment - create chrome namespace from browser
  window.chrome = {
    bookmarks: browser.bookmarks,
    storage: browser.storage,
    tabs: browser.tabs,
    runtime: browser.runtime,
    contextMenus: browser.contextMenus || browser.menus,
    i18n: browser.i18n
  };
}

// Enhanced i18n compatibility for Firefox
if (typeof chrome !== 'undefined' && typeof browser !== 'undefined') {
  // Ensure chrome.i18n works properly in Firefox
...
const FIREFOX_BOOKMARK_FOLDERS = {
  ...
  // Alternative IDs that might work in Firefox
  root: "root________",
  mobile: "mobile______"
};

// Simplified Firefox bookmark API compatibility
const FirefoxBookmarkAPI = {
  async getTree() {
    logDebug('FirefoxBookmarkAPI.getTree() called');
    
    // Use the appropriate API namespace
    const api = typeof browser !== 'undefined' && browser.bookmarks ? browser : chrome;
    
    if (!api || !api.bookmarks) {
      throw new Error('No bookmark API available');
    }
    ...
    try {
      ...
      const result = await Promise.race([
        new Promise((resolve, reject) => {
          api.bookmarks.getTree(nodes => {
            if (api.runtime?.lastError) {
              reject(new Error(api.runtime.lastError.message || 'Bookmark API error'));
            } else {
              resolve(nodes);
            }
          });
        ...
        new Promise((_, reject) => {
          setTimeout(() => reject(new Error('Bookmark API timeout')), 3000);
        })
      ]);

      if (result && result.length > 0) {
        logDebug('Bookmark tree retrieved successfully:', result.length, 'root nodes');
        return result;
      } else {
        throw new Error('Empty bookmark tree returned');
      }
    } catch (error) {
      logDebug('Bookmark API failed:', error);
      
      // Fallback: return minimal Firefox structure
      if (isFirefox()) {
        logDebug('Using Firefox fallback structure');
        return [{
          id: "0",
          title: "",
          children: [
...
function isFirefox() {
  ...
  return typeof browser !== 'undefined' && browser !== chrome || 
         (navigator && navigator.userAgent && navigator.userAgent.toLowerCase().includes('firefox')) ||
         (typeof InstallTrigger !== 'undefined') || // Firefox-specific
         (typeof window !== 'undefined' && window.InstallTrigger !== undefined);
}
...
async function loadAndStoreAllBookmarkFolders() {
    console.log("[BookStaxx Background] 모든 북마크 폴더 로드 및 저장 시작");
    ...
    try {
        ...
        try {
            ...
            console.log("[BookStaxx Background] 북마크 API 체크 시작");
            if (!chrome.bookmarks) {
                console.log("[BookStaxx Background] chrome.bookmarks API 없음!");
                throw new Error("북마크 권한이 없거나 API를 사용할 수 없습니다");
            }
            
            logDebug("Firefox 감지됨:", isFirefox());
            logDebug("사용 가능한 북마크 API:", {
                hasGetTree: typeof chrome.bookmarks.getTree === 'function',
                hasGetSubTree: typeof chrome.bookmarks.getSubTree === 'function',
                hasBrowserBookmarks: typeof browser !== 'undefined' && typeof browser.bookmarks !== 'undefined'
            });
            
            // Use enhanced Firefox bookmark API
            console.log("[BookStaxx Background] FirefoxBookmarkAPI.getTree() 호출 시작");
            treeNodes = await FirefoxBookmarkAPI.getTree();
            console.log("[BookStaxx Background] FirefoxBookmarkAPI.getTree() 결과:", treeNodes ? treeNodes.length : 'null');
            
            if (!treeNodes || treeNodes.length === 0) {
                throw new Error("북마크 트리가 비어있거나 null입니다");
            }
            
            logDebug(`북마크 getTree 성공: ${treeNodes.length}개 루트 노드`);
            
        } catch (error) {
            console.log("[BookStaxx Background] 북마크 API 접근 실패:", error);
            logDebug("모든 북마크 API 접근 방법 실패:", error);
            throw error;
        }
        ...
        console.log("[BookStaxx Background] 모든 북마크 폴더 로드 및 저장 중 오류:", error);
...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
  ...
  try {
    ...
    // 최상위 북마크 폴더 가져오기 - 간소화된 Firefox 처리
    if (request.action === "getTopLevelFolders") {
        console.log('[Background] getTopLevelFolders 요청 받음');
        
        (async () => {
            try {
                const tree = await FirefoxBookmarkAPI.getTree();
                const folders = [];
                
                function traverse(nodes, level = 0) {
                    for (const node of nodes) {
                        if (!node.url && node.id !== '0') {
                            let title = node.title || 'Unnamed Folder';
                ...
                traverse(tree);
                console.log('[Background] 폴더 가져오기 성공:', folders.length);
                sendResponse({ success: true, folders: folders });
                
            } catch (error) {
                console.log('[Background] 폴더 가져오기 실패:', error);
                // 완전 실패 시 기본 폴더 반환
                ...
                sendResponse({ success: true, folders: fallbackFolders });
            }
        })();
    ...
    if (request.action === "importInitialBookmarks") {
      if (!request.sourceFolderIds || !Array.isArray(request.sourceFolderIds) || request.sourceFolderIds.length === 0) {
        sendResponse({ success: false, error: "Invalid folder IDs for import." });
        return true;
      }
      ...
      if (!bookStaxxFolderId) {
        initBookStaxxFolder()
          ...
          .catch(error => {
            sendResponse({ success: false, error: "BookStaxx folder initialization failed: " + error.message });
          });
      } else {
    ...
    function importBookmarks(sourceFolderIds, sendResponse) {
      ...
      for (const folderId of sourceFolderIds) {
        chrome.bookmarks.getChildren(folderId, function(bookmarks) {
          if (chrome.runtime.lastError) {
            console.error("Bookmark import failed:", chrome.runtime.lastError);
            processedFolders++;
            checkCompletion();
            return;
          }
          ...
          for (const bookmark of bookmarksToImport) {
            ...
            chrome.bookmarks.create({
              ...
              if (chrome.runtime.lastError) {
                console.error("Bookmark creation failed:", chrome.runtime.lastError, bookmark.url);
              } else {
                importedCount++;
              }
    ...
    try {
      sendResponse({ success: false, error: "Message processing error: " + (error.message || "Unknown error") });
    } catch (responseError) {
      console.error("Response sending error:", responseError);
    }
    return true;
...
if (DEBUG_MODE) {
  ...
  // Firefox 환경 상세 진단
  if (isFirefox()) {
    logDebug("Firefox 환경 진단", {
      browserBookmarks: typeof browser !== 'undefined' && typeof browser.bookmarks !== 'undefined',
      chromeBookmarks: typeof chrome !== 'undefined' && typeof chrome.bookmarks !== 'undefined',
      bookmarkMethods: {
        getTree: typeof (browser?.bookmarks?.getTree || chrome?.bookmarks?.getTree) === 'function',
        search: typeof (browser?.bookmarks?.search || chrome?.bookmarks?.search) === 'function',
        getSubTree: typeof (browser?.bookmarks?.getSubTree || chrome?.bookmarks?.getSubTree) === 'function'
      }
    });
  }
}
