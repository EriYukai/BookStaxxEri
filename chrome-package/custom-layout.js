...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function calculateLayoutDimensions() {
        ...
        // 레이아웃 영역을 컨테이너 전체 크기로 설정하여 최대 사용 가능 공간 활용
    ...
    function drag(e) {
        ...
        // 실제 레이아웃 영역(editorContainer) 경계 내부에서의 새 위치 계산
        const layoutRect = editorContainer.getBoundingClientRect();
        let newX = e.clientX - layoutRect.left - offsetX;
        let newY = e.clientY - layoutRect.top - offsetY;

        // 경계 제한 - 레이아웃 영역의 실제 크기 사용
        const itemSize = draggingItem.offsetWidth; // 실제 아이템 크기 사용
        const maxX = editorContainer.offsetWidth - itemSize;
        const maxY = editorContainer.offsetHeight - itemSize;
