...
function handleFileInput(inputElement, storageKey) {
    ...
    optimizeImage(file, storageKey)
        .then(optimizedDataUrl => {
            ...
            splitAndStoreBase64(optimizedDataUrl, storageKey)
                .then(() => {
                    ...
                    showStatusMessage(`이미지가 최적화되어 저장되었습니다.`, 'success');
                })
                .catch(error => {
        ...
        .catch(error => {
            ...
            showStatusMessage(`이미지 최적화 중 오류가 발생했습니다: ${error.message}`, 'error');
        });
}
...
function updateSelectedFoldersCount() {
    ...
    // 선택된 폴더 개수 표시 업데이트
    selectedFoldersCount.innerHTML = `<span data-i18n="selectedFoldersLabel">${getLocalizedMessage('selectedFoldersLabel') || 'Selected folders:'}</span> <span class="text-bookmark-blue dark:text-teal-300 font-medium">${count}</span><span data-i18n="foldersUnit">${getLocalizedMessage('foldersUnit') || ''}</span>`;
...
function loadImportFolderListImpl() {
    ...
    const loadingHtml = `
        <div class="py-6 text-center text-gray-400">
            ...
            <p class="text-lg" data-i18n="loadingFolders">${chrome.i18n.getMessage('loadingFolders') || '북마크 폴더를 로드 중입니다...'}</p>
        </div>`;
    
    if (importFolderListDiv) {
        importFolderListDiv.innerHTML = loadingHtml;
    }
    if (selectedFoldersCount) selectedFoldersCount.innerHTML = `<span data-i18n="selectedFoldersLabel">${getLocalizedMessage('selectedFoldersLabel') || 'Selected folders:'}</span> <span class="text-blue-500">0</span><span data-i18n="foldersUnit">${getLocalizedMessage('foldersUnit') || ''}</span>`;
    ...
    const messageTimeout = setTimeout(() => {
        ...
        if (importFolderListDiv) importFolderListDiv.innerHTML = `<p class="py-3 text-center text-red-500 dark:text-red-400">${timeoutMsg}</p>`;
    ...
    try {
        chrome.runtime.sendMessage({ action: "getTopLevelFolders" }, (response) => {
            ...
            if (chrome.runtime.lastError) {
                console.error("[options.js] 폴더 로딩 오류 (sendMessage callback):", chrome.runtime.lastError);
                const errorMsg = chrome.i18n.getMessage("errorLoadingFolders", [chrome.runtime.lastError.message]) || `북마크 폴더 로딩 중 오류: ${chrome.runtime.lastError.message}`;
                if (importFolderListDiv) importFolderListDiv.innerHTML = `<p class="py-3 text-center text-red-500 dark:text-red-400">${errorMsg}</p>`;
                showImportStatus(errorMsg, 'error');
                return;
            }

            if (response && response.success && response.folders) {
                console.log("[options.js] populateImportFolderList 호출 전, 수신된 폴더 개수:", response.folders.length);
                populateImportFolderList(response.folders);
                updateSelectedFoldersCount(); 
                console.log("[options.js] populateImportFolderList 호출 완료.");
                ...
                console.error("[options.js] 폴더 가져오기 실패 또는 폴더 없음. 응답:", response);
                const noFoldersMsg = getLocalizedMessage("errorNoFoldersToImport") || chrome.i18n.getMessage("errorNoFoldersToImport") || '가져올 북마크 폴더가 없거나 로드에 실패했습니다.';
                if (importFolderListDiv) importFolderListDiv.innerHTML = `<p class="py-3 text-center text-red-500 dark:text-red-400">${noFoldersMsg}</p>`;
        ...
        if (importFolderListDiv) importFolderListDiv.innerHTML = `<p class="py-3 text-center text-red-500 dark:text-red-400">${exceptionMsg}</p>`;
...
function populateImportFolderList(folders) {
    console.log("[options.js] populateImportFolderList 시작, 전달된 폴더 수:", folders ? folders.length : 0);
    importFolderListDiv.innerHTML = ''; 
    if (!folders || folders.length === 0) {
        const noFolderKey = "noFoldersToImportMsg"; // 메시지 키 사용
        const noFolderMessage = document.createElement('p');
        noFolderMessage.textContent = chrome.i18n.getMessage(noFolderKey) || '가져올 북마크 폴더가 없습니다.';
    ...
    // 폴더가 있으면 버튼 활성화 (초기 상태)
    if(startImportButton) startImportButton.disabled = true; // 처음엔 선택된 폴더가 없으므로 비활성화
    ...
    headerElement.innerHTML = `<p class="text-sm text-gray-600 dark:text-gray-400" data-i18n="importBookmarksDesc">${getLocalizedMessage('importBookmarksDesc') || 'Import bookmarks from the folders below. Selecting a folder will import all bookmarks from that folder into BookStaxx.'}</p>`;
    ...
    folders.forEach(folder => {
        ...
        if (folder.id === '1') {
            iconSpan.innerHTML = '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"></path></svg>';
        } else if (folder.id === '2') {
            iconSpan.innerHTML = '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2v-2"></path></svg>';
        } else {
            iconSpan.innerHTML = '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"></path></svg>';
        }
        const textContainer = document.createElement('div');
        textContainer.className = 'flex flex-col flex-grow overflow-hidden';
        const textSpan = document.createElement('span');
        textSpan.textContent = folder.title || (folder.id === '1' ? (chrome.i18n.getMessage('bookmarksBarLabel') || '북마크 바') : (chrome.i18n.getMessage('otherBookmarksLabel') || '기타 북마크'));
    ...
    if (folderCount === 0 && importFolderListDiv.childElementCount <= 1) { // 헤더만 있을 경우
        ...
        const noFolderMessage = document.createElement('p');
        noFolderMessage.textContent = chrome.i18n.getMessage(noOtherFoldersKey) || 'BookStaxx 폴더를 제외한 가져올 북마크 폴더가 없습니다.';
        noFolderMessage.className = 'py-3 text-center text-gray-600 dark:text-gray-400';
        importFolderListDiv.appendChild(noFolderMessage);
    }
    if (selectedFoldersCount) selectedFoldersCount.innerHTML = `<span data-i18n="selectedFoldersLabel">${getLocalizedMessage('selectedFoldersLabel') || 'Selected folders:'}</span> <span class="text-blue-500">0</span><span data-i18n="foldersUnit">${getLocalizedMessage('foldersUnit') || ''}</span>`;
...
function handleStartImport() {
    ...
    startImportButton.innerHTML = `
        ...
        ${getLocalizedMessage('importing') || 'Importing...'}
    `;
    ...
    chrome.runtime.sendMessage({ action: "importInitialBookmarks", sourceFolderIds: folderIdsToImport },
        (response) => {
            ...
            if (chrome.runtime.lastError) {
                console.error("Bookmark import error:", chrome.runtime.lastError);
...
function updateDynamicMessages() {
    ...
    errorMessages.forEach(elem => {
        ...
        if (text.includes('가져올 북마크 폴더가 없거나') || text.includes('not found') || text.includes('インポート')) {
            const newMsg = getLocalizedMessage("errorNoFoldersToImport") || chrome.i18n.getMessage("errorNoFoldersToImport") || '가져올 북마크 폴더가 없거나 불러올 수 없습니다.';
    ...
    loadingMessages.forEach(elem => {
        const newMsg = getLocalizedMessage("loadingFolders") || chrome.i18n.getMessage("loadingFolders") || '북마크 폴더를 로드 중입니다...';
        elem.textContent = newMsg;
    });
    
    // 버튼 텍스트 업데이트
    if (startImportButton) {
        const btnMsg = getLocalizedMessage("startImport") || chrome.i18n.getMessage("startImport") || '가져오기 시작';
